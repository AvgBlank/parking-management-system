...
async function main() {
  ...
  const v1 = await prisma.vehicle.create({
    data: { licensePlate: "JANE-123", model: "Toyota Camry", userId: jane.id },
  });
  const v2 = await prisma.vehicle.create({
    data: { licensePlate: "JANE-EV", model: "Tesla Model 3", userId: jane.id },
  });
  const v3 = await prisma.vehicle.create({
    data: { licensePlate: "BOB-TRK", model: "Ford F-150", userId: bob.id },
  });
  const v4 = await prisma.vehicle.create({
    data: { licensePlate: "ALI-001", model: "Honda Civic", userId: alice.id },
  });
  ...
  for (let i = 0; i < 5; i++) {
    await prisma.booking.create({
      data: {
        startTime: addHours(now, -48 + i),
        endTime: addHours(now, -46 + i),
        status: BookingStatus.completed,
        userId: bob.id,
        vehicleId: v3.id,
        slotId: flatSlotsA[10 + i].id,
        payments: {
          create: {
            amount: 10.0,
            status: PaymentStatus.completed,
            paidAt: addHours(now, -48 + i),
            userId: bob.id,
          },
        },
      },
  ...
  await prisma.booking.create({
    data: {
      ...
      userId: alice.id,
      vehicleId: v4.id,
      slotId: flatSlotsA[5].id, // Slot freed
      payments: {
        create: {
          amount: 10.0,
  ...
  console.log(
    `✅ Generated 11 Bookings across varying lifecycle states & $105+ in registered payments`,
  );
